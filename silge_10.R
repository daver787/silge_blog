...
scooby_raw %>%
  filter(monster_amount>0)%>%
  count(year_aired=5*(lubridate::year(date_aired)%%5),monster_real)%>%
  ggplot(aes(year_aired,n,fill=monster_real))+
  geom_col(position=position_dodge(preserve="single"),alpha=0.8)+
  labs(x="Date aired",y="Monsters per 5 years",fill="Real monster?")
...
scooby_split <- scooby_raw%>%
  ...
  mutate(monster_real =case_when(monster_real=="FALSE"~"fake",TRUE~"real"),
