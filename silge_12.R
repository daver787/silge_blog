...
##Create recipe and model spec and fit
library(tidymodels)
...
set.seed(2020)
lasso_grid <-tune_grid(wf%>%update_model(tune_spec),
...
lowest_rmse<- lasso_grid%>%
  select_best("rmse",maximimze="FALSE")

final_lasso <- finalize_workflow(wf%>%update_model(tune_spec),lowest_rmse)
